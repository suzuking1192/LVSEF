...
def without_retro(generated_samples, args):
    ...
    for sample_smi in generated_samples:
        ...
        try:
            ...
            result = syn.planner.plan(sample_smi)
        except:
            result = None
        ...
        for num in range(args.MCMC_size):
            ...
            if args.grammar_training:
                ...
                if train_epoch == 0 and num == 0:
                    markov_grammar = markov_grammar_model(l_grammar, args.grammar_lr)
                markov_grammar.adjust_q_table_columns(l_grammar)
                markov_grammar.train_markov_grammar_by_reconstruction(l_grammar)
                for i in range(args.grammar_training_round):
                    markov_grammar.train_markov_grammar(l_grammar)
                eval_metric = evaluate_by_trained_grammar(l_grammar, args, metrics=["diversity", "syn"])
            ...
            if R_ind > curr_max_R:
                ...
                if args.grammar_training:
                    markov_grammar_filename = "{}/epoch_input_graphs_{}_{}.pkl".format(save_log_path, train_epoch, R_ind)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--grammar_training_round", type=int, default=10, help="grammar training epochs")
