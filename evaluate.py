...
def load_and_generate_mols(args):
    ...
    if args.save_grammar_rule_png:
        save_log_path = 'log/log-evaluate_{}-{}_grammar_training_{}_expr_name_{}_{}'.format(args.num_generated_samples, time.strftime("%Y%m%d-%H%M%S"),args.grammar_training,args.expr_name,args.task_name)
    ...
    for i in range(args.num_generated_samples):
        if args.grammar_training and args.random_combination == False:
            mol, iter_num, _ = markov_grammar.produce(grammar,args.uniform_smoothing_e,args.a_ending_factor,args.top_k_selection,args.random_iter_start)
...
def evaluate_and_save_log(generated_mols,args,grammar):
    ...
    if args.save_sa_score:
        ...
        num_useful_mols, useful_validity, useful_uniqueness, useful_diversity, useful_chamfer,useful_smi_list = calculate_useful_mol_percentage(generated_mols,mol_sml)
        
        
        # number of useful total molecules without considering membership
        
        num_useful_mols_without_membership, useful_validity_without_membership, useful_uniqueness_without_membership, useful_diversity_without_membership, useful_chamfer_without_membership,useful_smi_list = calculate_useful_mol_percentage(generated_mols,mol_sml,without_membership=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--random_combination', action="store_true", default=False, help="add another top k from q table randomly for baseline comparison")
    ...
    parser.add_argument('--top_k_based_on_chosen_ones', action="store_true", default=False, help="add another top k from q table randomly for baseline comparison")
    ...
    parser.add_argument('--save_grammar_sample_smi', action="store_true", default=False, help="add another top k from q table randomly for baseline comparison")
    parser.add_argument('--save_grammar_rule_png', action="store_true", default=False, help="add another top k from q table randomly for baseline comparison")
    
    parser.add_argument('--save_sa_score', action="store_true", default=False, help="add another top k from q table randomly for baseline comparison")
    
    parser.add_argument('--generated_smi_file', type=str,default=None, help="task name")
    
    parser.add_argument('--select_frag', action="store_true", default=False, help="add another top k from q table randomly for baseline comparison")
