...
class markov_grammar_model:
    ...
    def produce(self,grammar,uniform_smoothing_e=0,a=0.5,top_k_selection=0,random_iter_start=15):
        ...
        def prob_schedule(_iter, selected_idx,last_rule_idx,uniform_smoothing_e=0,a=0.5,top_k_selection=0,random_iter_start=15):
            prob_list = []
            ...
            if uniform_smoothing_e > 0:
                try:
                    ...
                    smoothed_probabilities = [(p + uniform_smoothing_e) / (1 + num_events * uniform_smoothing_e) for p in prob_list]
        ...
        while(True):
            if iter == 0:
                ...
                if (all([rl.is_start_rule for rl in candidate_rule]) and iter > 0) or iter > 30:
                    break
                prob_list = prob_schedule(iter, candidate_rule_idx,last_rule_idx,uniform_smoothing_e,a,top_k_selection,random_iter_start)
