from private.hypergraph import mol_to_hg_add_dummy
...
def learn(smiles_list, args):
    # Create logger
    save_log_path = "log/log-num_generated_samples{}-{}_grammar_training_{}_grammar_lr_{}_grammar_explore_rate{}_{}".format(
        args.num_generated_samples,
        time.strftime("%Y%m%d-%H%M%S"),
        args.grammar_training,
        args.grammar_lr,
        args.grammar_explore_rate,
        args.task_name,
    )
    ...
    if args.select_fragments:
        ...
        markov_grammar = markov_grammar_model(
            frag_grammar,
            args.grammar_lr,
            args.grammar_explore_rate,
            args.grammar_reconstruction_addition,
            True,
            valid_smi_list,
        )
        ...
        for num in range(args.MCMC_size):
            ...
            if args.fragment_ranking_pred:
                ...
                if train_epoch == 0 and num == 0:
                    markov_grammar = markov_grammar_model(
                        l_grammar, args.grammar_lr, args.grammar_explore_rate, args.grammar_reconstruction_addition
                    )
                    iter_num, l_grammar, l_input_graphs_dict = MCMC_sampling(
                        ...
                        num,
                        args,
                        markov_model=markov_grammar,
                        gammar=args.fragment_ranking_pred_gammar**total_step,
            ...
            if args.grammar_training:
                ...
                if args.fragment_ranking_pred:
                    ...
                    if R_ind > curr_max_R or total_step == (args.max_epoches * args.MCMC_size - 1) or total_step % 50 == 0:
                        torch.save(
                            agent.state_dict(), os.path.join(save_log_path, "epoch_agent_{}_{}.pkl".format(total_step, R_ind))
                        )
                ...
                for i in range(args.grammar_training_round):
                    diversity, syn = markov_grammar.train_markov_grammar(
                        frag_grammar, args, args.grammar_training_round_sample
                    )
                    logger.info(f"markov model training results=== diversity: {diversity}, synthesizability: {syn}")
            ...
            if args.fragment_ranking_pred or args.select_fragments:
                ...
                if R_ind > curr_max_R:
                    torch.save(
                        agent.state_dict(), os.path.join(save_log_path, "epoch_agent_{}_{}.pkl".format(train_epoch, R_ind))
                    )
            ...
            for sample_number in agent.saved_log_probs.keys():
                ...
                for iter_num_key in agent.saved_log_probs[sample_number].keys():
                    ...
                    for log_prob in log_probs:
                        policy_loss += (
                            -log_prob * args.gammar ** (max_iter_num - iter_num_key) * returns[sample_number]
                        ).sum()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--motif", action="store_true", default=False, help="use motif as the basic building block for polymer dataset"
    )
    ...
    parser.add_argument(
        "--grammar_reconstruction_addition",
        type=float,
        default=0.5,
        help="reconstruction addition reward for grammar training",
    )

    parser.add_argument(
        "--without_reconstruction", action="store_true", default=False, help="train grammar without using reconstruction loss"
    )
    # without retrostar
    ...
    parser.add_argument(
        "--fragment_ranking_pred", action="store_true", default=False, help="predict bond cutting based on q table"
    )
