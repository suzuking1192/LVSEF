...
from private.utils import _edge_match, _node_match_prod_rule, masked_softmax
...
class ProductionRule(object):
    ...
    def is_same(self, prod_rule, ignore_order=False):
        ...
        subhg_bond_symbol_counter = Counter(
            [prod_rule.rhs.node_attr(each_node)["symbol"] for each_node in prod_rule.rhs.nodes]
        )
        ...
        subhg_atom_symbol_counter = Counter(
            [prod_rule.rhs.edge_attr(each_edge)["symbol"] for each_edge in prod_rule.rhs.edges]
        )
    ...
    def graph_rule_applied_to(self, input_hg, selected_edge=None, selected_iso_mapping=None, vis=False):
        ...
        if self.is_start_rule:
            ...
            for num_idx, each_node in enumerate(self.rhs.nodes):
                hg.add_node(f"bond_{num_idx+node_idx}", attr_dict=self.rhs.node_attr(each_node))
            ...
            for each_node in self.rhs.nodes:
                if "ext_id" in self.rhs.node_attr(each_node):
                    ...
                    node_map_rhs[each_node] = nt_order_dict_inv[self.rhs.node_attr(each_node)["ext_id"]]
                else:
            ...
            for each_edge in self.rhs.edges:
                ...
                edge_id = hg.add_edge(
                    node_list_hg, attr_dict=self.rhs.edge_attr(each_edge)
                )  # deepcopy(self.rhs.edge_attr(each_edge)))
                if "nt_idx" in hg.edge_attr(edge_id):
                    nt_edge_dict[hg.edge_attr(edge_id)["nt_idx"]] = edge_id
            nt_edge_list = [nt_edge_dict[key] for key in range(len(nt_edge_dict))]
    ...
    def applied_to(self, hg: Hypergraph, edge: str) -> Tuple[Hypergraph, List[str]]:
        """augment `hg` by replacing `edge` with `self.rhs`.
    ...
    def revert(self, hg: Hypergraph, return_subhg=False):
        ...
        """
        gm = GraphMatcher(hg.hg, self.rhs.hg, node_match=_node_match_prod_rule, edge_match=_edge_match)
        try:
        ...
        if return_subhg:
            ...
            for each_edge in hg.edges:
                if each_edge in isomap:
                    subhg.add_edge(hg.nodes_in_edge(each_edge), attr_dict=hg.edge_attr(each_edge), edge_name=each_edge)
        ...
        hg.add_edge(nt_node_list, attr_dict=dict(terminal=False, symbol=self.lhs_nt_symbol))
        if return_subhg:
...
class ProductionRuleCorpus(object):
    ...
    def construct_feature_vectors(self):
        ...
        for each_edge_symbol in self.edge_symbol_list:
            ...
            feature = torch.sparse.LongTensor(
                torch.LongTensor([idx_list]), torch.ones(len(idx_list)), torch.Size([len(feature_id_dict)])
            )
        ...
        for each_ext_id in self.ext_id_list:
            ...
            feature_dict[("ext_id", each_ext_id)] = torch.sparse.LongTensor(
                torch.LongTensor([idx_list]), torch.ones(len(idx_list)), torch.Size([len(feature_id_dict)])
            )
                ...
                for key, val in isomap.items():  # key : edges & nodes in each_prod_rule.rhs , val : those in prod_rule.rhs
                    ...
                    if "nt_idx" in prod_rule.rhs.edge_attr(val).keys():
                        if "nt_idx" not in each_prod_rule.rhs.edge_attr(key).keys():
                            raise ValueError
                        prod_rule.rhs.set_edge_attr(val, {"nt_idx": each_prod_rule.rhs.edge_attr(key)["nt_idx"]})
    ...
    def sample(self, unmasked_logit_array, nt_symbol, deterministic=False):
        ...
        if deterministic:
            prob = masked_softmax(
                unmasked_logit_array, self.lhs_in_prod_rule[self.nt_symbol_list.index(nt_symbol)].numpy().astype(np.float64)
            )
    ...
    for each_edge in prod_rule.rhs.edges:
        if "nt_idx" in prod_rule.rhs.edge_attr(each_edge).keys():
            for each_child in children:
                common_node_set = set(
                    common_node_list(clique_tree.node[myself]["subhg"], clique_tree.node[each_child]["subhg"])[0]
                )
...
def generate_rule(input_graph, subg, grammar):
    ...
    watershed_mapping = {}
    all_nodes_subg = [
        input_graph.get_org_node_in_input(node, subg) for node in subg_hyper_graph.nodes
    ]  # TODO a stupid implementation
    ...
    for water_level, node_set in watershed_mapping.items():
        ...
        edge_attr_dict = dict(
            terminal=False, symbol=NTSymbol(degree=len(node_list), is_aromatic=is_aromatic, bond_symbol_list=bond_symbol_list)
        )
        rhs.add_edge(node_list, attr_dict=edge_attr_dict)
    try:
    ...
    if not starting:
        ...
        lhs.add_edge(
            ...
            attr_dict=dict(
                ...
                symbol=NTSymbol(degree=len(ext_node_list), is_aromatic=is_aromatic, bond_symbol_list=bond_symbol_list),
            ),
        )
        try:
...
def extract_prod_rule(parent_hg, myself_hg, children_hg_list, subhg_idx=None):
    ...
    if parent_hg is None:
        ...
        for each_node in node_list:
            lhs.add_node(each_node, deepcopy(myself_hg.node_attr(each_node)))
        ...
        lhs.add_edge(
            ...
            attr_dict=dict(
                ...
                edge_exists=edge_exists,
                symbol=NTSymbol(
                    degree=len(node_list), is_aromatic=is_aromatic, bond_symbol_list=bond_symbol_list, for_ring=for_ring
                ),
            ),
        )
        try:
    ...
    if DEBUG:
        if sorted(list(prod_rule.ext_node.keys())) != list(np.arange(len(prod_rule.ext_node))):
