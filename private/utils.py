...
def _easy_node_match(node1, node2):
    ...
    if node1["bipartite"] == "edge" and node2["bipartite"] == "edge":
        ...
        # bond_symbol
        return (
            node1["attr_dict"].get("ext_id", -1) == node2["attr_dict"].get("ext_id", -1)
...
def _node_match_prod_rule(node1, node2, ignore_order=False):
    ...
    if node1["bipartite"] == "edge" and node2["bipartite"] == "edge":
        ...
        if ignore_order:
            ...
            return node1["attr_dict"]["symbol"] == node2["attr_dict"]["symbol"] and node1["attr_dict"].get(
                "ext_id", -1
            ) == node2["attr_dict"].get("ext_id", -1)
    else:
        return False
