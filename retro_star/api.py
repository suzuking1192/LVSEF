...
class RSPlanner:
    def __init__(
        ...
        if use_value_fn:
            model = ValueMLP(n_layers=1, fp_dim=fp_dim, latent_dim=128, dropout_rate=0.1, device=device).to(device)
    ...
    def plan(self, target_mol):
        ...
        if succ:
            ...
            logging.info("Synthesis path for %s not found. Please try increasing " "the number of iterations." % target_mol)
            return None


if __name__ == "__main__":
    planner = RSPlanner(gpu=0, use_value_fn=True, iterations=100, expansion_topk=50)
