    ...
    for mol in mols:
        ...
        for k in range(radius):
            atoms_to_use, symbol_replacements = expand_atoms_to_use(
                mol, atoms_to_use, groups=groups, symbol_replacements=symbol_replacements
            )
...
def extract_from_reaction(reaction):
    ...
    if are_unmapped_product_atoms:  # add fragment to template
        for product in products:
            # Get unmapped atoms
            unmapped_ids = [a.GetIdx() for a in prod_atoms if not a.HasProp("molAtomMapNumber")]
