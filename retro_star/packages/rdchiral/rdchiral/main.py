    ...
    for outcome in outcomes:
        ...
        for atoms, dirs, is_implicit in reactants.atoms_across_double_bonds:
            if all(i in atoms_rt for i in atoms):
                ...
                if (
                    dirs != dirs_template
                    and (BondDirOpposite[dirs[0]], BondDirOpposite[dirs[1]]) != dirs_template
        ...
        if len(mapnums) != len(set(mapnums)):  # duplicate?
            ...
            for j in range(1, len(outcome)):
                ...
                for b in new_mol.GetBonds():
                    ...
                    if not merged_mol.GetBondBetweenAtoms(merged_map_to_id[bi], merged_map_to_id[bj]):
                        merged_mol.AddBond(merged_map_to_id[bi], merged_map_to_id[bj], b.GetBondType())
                        merged_mol.GetBondBetweenAtoms(merged_map_to_id[bi], merged_map_to_id[bj]).SetStereo(b.GetStereo())
                        merged_mol.GetBondBetweenAtoms(merged_map_to_id[bi], merged_map_to_id[bj]).SetBondDir(b.GetBondDir())
        ...
        atoms_pt = {
            a.GetAtomMapNum(): atoms_pt_map[a.GetIntProp("old_mapno")] for a in outcome.GetAtoms() if a.HasProp("old_mapno")
        }
            ...
            if i in atoms_p and j in atoms_p:
                ...
                if not outcome.GetBondBetweenAtoms(atoms_p[i].GetIdx(), atoms_p[j].GetIdx()):
                    ...
                    if (
                        ...
                        or not template_r.GetBondBetweenAtoms(atoms_rt[i].GetIdx(), atoms_rt[j].GetIdx())
                    ):
        ...
        for a in outcome.GetAtoms():
            ...
            if not a.HasProp("old_mapno"):
                ...
                if not a.HasProp("react_atom_idx"):
                    ...
                    if PLEVEL >= 4:
                        print("Atom {} created by product template, should have right chirality".format(a.GetAtomMapNum()))

                else:
                    if PLEVEL >= 4:
                        print("Atom {} outside of template, copy chirality from reactants".format(a.GetAtomMapNum()))
                ...
                if template_atom_could_have_been_tetra(atoms_rt[a.GetAtomMapNum()]):
                    if PLEVEL >= 3:
                        print("Atom {} was in rct template (could have been tetra)".format(a.GetAtomMapNum()))

                    if template_atom_could_have_been_tetra(atoms_pt[a.GetAtomMapNum()]):
                        if PLEVEL >= 3:
                            print("Atom {} in product template could have been tetra, too".format(a.GetAtomMapNum()))
                        ...
                        if atoms_pt[a.GetAtomMapNum()].GetChiralTag() == ChiralType.CHI_UNSPECIFIED:
                            ...
                            if PLEVEL >= 3:
                                print("...but it is not specified in product, so destroy chirality")
                            ...
                            if atoms_rt[a.GetAtomMapNum()].GetChiralTag() == ChiralType.CHI_UNSPECIFIED:
                                ...
                                if PLEVEL >= 3:
                                    print("...but reactant template was not, so copy from product template")
                                copy_chirality(atoms_pt[a.GetAtomMapNum()], a)

                            else:
                                # Yes, so we need to check if chirality should be preserved or inverted
                                if PLEVEL >= 3:
                                    print("...and reactant template was, too! copy from reactants")
                                copy_chirality(atoms_r[a.GetAtomMapNum()], a)
                    ...
                    if PLEVEL >= 3:
                        print("Atom {} could not have been chiral in reactant template".format(a.GetAtomMapNum()))

                    if not template_atom_could_have_been_tetra(atoms_pt[a.GetAtomMapNum()]):
                        if PLEVEL >= 3:
                            print("Atom {} also could not have been chiral in product template", a.GetAtomMapNum())
                        if PLEVEL >= 3:
                        ...
                        if PLEVEL >= 3:
                            print("Atom could/does have product template chirality!".format(a.GetAtomMapNum()))
                        if PLEVEL >= 3:
        ...
        if skip_outcome:
            ...
            continue
        if PLEVEL >= 2:
            print("After attempting to re-introduce chirality, outcome = {}".format(Chem.MolToSmiles(outcome, True)))
        ###############################################################################
        ...
        for b in outcome.GetBonds():
            ...
            if PLEVEL >= 5:
                print("Looking at outcome bond {}={}".format(ba.GetAtomMapNum(), bb.GetAtomMapNum()))
            ...
            if ba.HasProp("old_mapno") and bb.HasProp("old_mapno"):
                ...
                if PLEVEL >= 5:
                    print("Both atoms in this double bond were in the reactant template")
                if (ba.GetIntProp("old_mapno"), bb.GetIntProp("old_mapno")) in rxn.required_bond_defs_coreatoms:
                    if PLEVEL >= 5:
                        print("and reactant template *could* have specified the chirality!")
                    if PLEVEL >= 5:
                        print("..product should be property instantiated")
                    continue
                if PLEVEL >= 5:
                    print("But it was impossible to have specified chirality (e.g., aux C=C for context)")
            ...
            if PLEVEL >= 5:
                print(
                    "Restoring cis/trans character of bond {}={} from reactants".format(ba.GetAtomMapNum(), bb.GetAtomMapNum())
                )
        ...
        if len(tetra_copied_from_reactants) > 0:
            ...
            for a in tetra_copied_from_reactants:
                if a.GetChiralTag() == ChiralType.CHI_UNSPECIFIED:
                    if PLEVEL >= 2:
                        print("Auxiliary reactant atom was chiral, now is broken -> skip outcome")
                    skip_outcome = True
                    break
        if skip_outcome:
            continue
...
if __name__ == "__main__":
    ...
    reactant_smiles = (
        "[CH3:5][C:6]1[CH2:28][S:27][C@@H:9]2[C@H:10]([NH2:13])[C:11](=[O:12])[N:8]2[C:7]=1[C:29]([OH:31])=[O:30]"
    )
    outcomes = rdchiralRunText(reaction_smarts, reactant_smiles)
    print(outcomes)
