...
def get_changed_atoms(reactants, products):
    ...
    if v:
        print(
            "Found {} atom-mapped tetrahedral atoms that have chirality specified at least partially".format(len(tetra_atoms))
        )
    ...
    for atom_tag, ar, ap in tetra_atoms:
        ...
        if atom_tag in changed_atom_tags:
            ...
            unchanged = check_tetrahedral_centers_equivalent(ar, ap) and ChiralType.CHI_UNSPECIFIED not in [
                ar.GetChiralTag(),
                ap.GetChiralTag(),
            ]
            if unchanged:
        ...
        if not super_general:
            for neighbor in atom.GetNeighbors():
                ...
                new_atoms_to_use, symbol_replacements = expand_atoms_to_use_atom(
                    mol, new_atoms_to_use, neighbor.GetIdx(), groups=groups, symbol_replacements=symbol_replacements
                )
            ...
            for neighbor in atom.GetNeighbors():
                if neighbor.GetIdx() not in new_atoms_to_use:
                    ...
                    symbol_replacements.append(
                        (neighbor.GetIdx(), convert_atom_to_wildcard(neighbor, super_general=super_general))
                    )
...
def get_frag_around_tetrahedral_center(mol, idx):
    ...
    symbols = [
        "[{}{}]".format(a.GetIsotope(), a.GetSymbol()) if a.GetIsotope() != 0 else "[#{}]".format(a.GetAtomicNum())
        for a in mol.GetAtoms()
    ]
    return Chem.MolFragmentToSmiles(
        mol, ids_to_include, isomericSmiles=True, atomSymbols=symbols, allBondsExplicit=True, allHsExplicit=True
    )
...
def get_fragments_for_changed_atoms(
    mols, changed_atom_tags, radius=0, category="reactants", expansion=[], super_general=False
):
    ...
    for mol in mols:
        ...
        for k in range(radius):
            atoms_to_use, symbol_replacements = expand_atoms_to_use(
                mol, atoms_to_use, groups=groups, symbol_replacements=symbol_replacements, super_general=super_general
            )
            ...
            if expansion:
                for atom in mol.GetAtoms():
                    ...
                    if label in expansion and label not in changed_atom_tags:
                        ...
                        symbol_replacements.append(
                            (atom.GetIdx(), convert_atom_to_wildcard(atom, super_general=super_general))
                        )
                        if v:
...
def extract_from_reaction(reaction, super_general=False, v=False):
    ...
    try:
        ...
        if are_unmapped_product_atoms:  # add fragment to template
            ...
            for product in products:
                # Get unmapped atoms
                unmapped_ids = [a.GetIdx() for a in product.GetAtoms() if not a.HasProp("molAtomMapNumber")]
        ...
        retro_rxn_canonical = (
            retro_rxn_canonical_split[0][1:-1].replace(").(", ".")
            + ">>"
        ...
        react_rxn_canonical = (
            react_rxn_canonical_split[0][1:-1].replace(").(", ".")
            + ">>"
