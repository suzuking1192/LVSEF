...
from rdchiral.old.chiral import atom_chirality_matches, copy_chirality, template_atom_could_have_been_tetra
from rdchiral.old.clean import canonicalize_outcome_smiles, combine_enantiomers_into_racemic
from rdchiral.old.initialization import rdchiralReactants, rdchiralReaction
...
def rdchiralRun(rxn, reactants, keep_isotopes=False, combine_enantiomers=True):
    ...
    for outcome in outcomes:
        ...
        atoms_rt = {
            a.GetIsotope(): atoms_rt_map[a.GetIntProp("old_mapno")]
            for m in outcome
            for a in m.GetAtoms()
        ...
        if len(isotopes) != len(set(isotopes)):  # duplicate?
            ...
            for j in range(1, len(outcome)):
                ...
                for b in new_mol.GetBonds():
                    ...
                    if not merged_mol.GetBondBetweenAtoms(merged_iso_to_id[bi], merged_iso_to_id[bj]):
                        merged_mol.AddBond(merged_iso_to_id[bi], merged_iso_to_id[bj], b.GetBondType())
                        merged_mol.GetBondBetweenAtoms(merged_iso_to_id[bi], merged_iso_to_id[bj]).SetStereo(b.GetStereo())
                        merged_mol.GetBondBetweenAtoms(merged_iso_to_id[bi], merged_iso_to_id[bj]).SetBondDir(b.GetBondDir())
        ...
        atoms_pt = {
            a.GetIsotope(): atoms_pt_map[a.GetIntProp("old_mapno")] for a in outcome.GetAtoms() if a.HasProp("old_mapno")
        }
                        ...
                        if atoms_pt[a.GetIsotope()].GetChiralTag() == ChiralType.CHI_UNSPECIFIED:
                            ...
                            if atoms_rt[a.GetIsotope()].GetChiralTag() == ChiralType.CHI_UNSPECIFIED:
                                ...
                                if not atom_chirality_matches(atoms_pt[a.GetIsotope()], atoms_rt[a.GetIsotope()]):
                                    vprint(
                                        3, "but! reactant template and product template have opposite stereochem, so invert"
                                    )
                                    a.InvertChirality()
        ...
        vprint(2, "After attempting to re-introduce chirality, outcome = {}", Chem.MolToSmiles(outcome, True))
        ###############################################################################
