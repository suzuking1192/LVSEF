...
from rdchiral.bonds import bond_dirs_by_mapnum, enumerate_possible_cistrans_defs, get_atoms_across_double_bonds
...
class rdchiralReaction:
    ...
    def __init__(self, reaction_smarts):
        ...
        self.atoms_rt_map = {a.GetAtomMapNum(): a for a in self.template_r.GetAtoms() if a.GetAtomMapNum()}
        self.atoms_pt_map = {a.GetAtomMapNum(): a for a in self.template_p.GetAtoms() if a.GetAtomMapNum()}

        # Back-up the mapping for the reaction
        self.atoms_rt_idx_to_map = {a.GetIdx(): a.GetAtomMapNum() for a in self.template_r.GetAtoms()}
        self.atoms_pt_idx_to_map = {a.GetIdx(): a.GetAtomMapNum() for a in self.template_p.GetAtoms()}
        ...
        self.required_rt_bond_defs, self.required_bond_defs_coreatoms = enumerate_possible_cistrans_defs(self.template_r)
...
class rdchiralReactants:
    ...
    def __init__(self, reactant_smiles):
        ...
        [(b.SetStereo(BondStereo.STEREONONE), b.SetBondDir(BondDir.NONE)) for b in self.reactants_achiral.GetBonds()]

        # Pre-list reactant bonds (for stitching broken products)
        self.bonds_by_mapnum = [
            (b.GetBeginAtom().GetAtomMapNum(), b.GetEndAtom().GetAtomMapNum(), b) for b in self.reactants.GetBonds()
        ]
...
def initialize_reactants_from_smiles(reactant_smiles):
    ...
    if PLEVEL >= 2:
        print("Initialized reactants, assigned map numbers, stereochem, flagpossiblestereocenters")
    return reactants
