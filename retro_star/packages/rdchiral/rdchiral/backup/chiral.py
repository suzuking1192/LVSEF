...
def copy_chirality(a_src, a_new):
    ...
    if a_new.GetDegree() < 3:
        return
    if a_new.GetDegree() == 3 and any(b.GetBondType() != BondType.SINGLE for b in a_new.GetBonds()):
        return

    if PLEVEL >= 3:
        print("For mapnum {}, copying src {} chirality tag to new".format(a_src.GetAtomMapNum(), a_src.GetChiralTag()))
...
def atom_chirality_matches(a_tmp, a_mol):
    ...
    if a_mol.GetChiralTag() == ChiralType.CHI_UNSPECIFIED:
        if a_tmp.GetChiralTag() == ChiralType.CHI_UNSPECIFIED:
            if PLEVEL >= 3:
                print("atom {} is achiral & achiral -> match".format(a_mol.GetAtomMapNum()))
        ...
        if not a_mol.HasProp("_ChiralityPossible"):
            ...
            if PLEVEL >= 3:
                print("atom {} is specified in template, but cant possibly be chiral in mol".format(a_mol.GetAtomMapNum()))
            return 2
        ...
        if PLEVEL >= 3:
            print("atom {} is achiral in mol, but specified in template".format(a_mol.GetAtomMapNum()))
        return 0
    if a_tmp.GetChiralTag() == ChiralType.CHI_UNSPECIFIED:
        if PLEVEL >= 3:
            print("Reactant {} atom chiral, rtemplate achiral...".format(a_tmp.GetAtomMapNum()))
        if template_atom_could_have_been_tetra(a_tmp):
            if PLEVEL >= 3:
                print("...and that atom could have had its chirality specified! no_match")
            return 0
        if PLEVEL >= 3:
            print("...but the rtemplate atom could not have had chirality specified, match anyway")
        return 2
    ...
    try:
        ...
        if len(only_in_src) <= 1 and len(only_in_mol) <= 1:
            ...
            if PLEVEL >= 2:
                print("mapnum {} chiral match? {}".format(a_tmp.GetAtomMapNum(), chirality_matches))
            return 1 if chirality_matches else -1
        else:
            if PLEVEL >= 2:
                print("mapnum {} chiral match? Based on mapnum lists, ambiguous -> True".format(a_tmp.GetAtomMapNum()))
