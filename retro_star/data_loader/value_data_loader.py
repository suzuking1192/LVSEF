...
def unpack_fps(packed_fps):
    ...
    fps = np.unpackbits(packed_fps.reshape((-1, packed_fps.shape[-1])), axis=-1)
...
class ValueDataset(Dataset):
    def __init__(self, fp_value_f):
        ...
        print(self.fps.shape, self.values.shape, self.reactant_fps.shape, self.reactant_masks.shape)
        ...
        logging.info(
            ...
            % (
                ...
                self.values.min(),
                self.values.max(),
                (self.values == 0).sum() * 1.0 / self.fps.shape[0],
            )
        )
    ...
    def reshuffle(self):
        ...
        self.reactant_fps = unpack_fps(self.reactant_packed_fps[: self.fps.shape[0], :, :])
    ...
    def __getitem__(self, index):
        return (
            self.fps[index],
            self.values[index],
            self.reaction_costs[index],
            self.target_values[index],
            self.reactant_fps[index],
            self.reactant_masks[index],
        )
...
class ValueDataLoader(DataLoader):
    def __init__(self, fp_value_f, batch_size, shuffle=True):
        ...
        super(ValueDataLoader, self).__init__(dataset=self.dataset, batch_size=batch_size, shuffle=shuffle)
