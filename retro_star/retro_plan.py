...
from retro_star.common import args, prepare_mlp, prepare_molstar_planner, prepare_starting_molecules, smiles_to_fp
...
def retro_plan():
    ...
    if args.use_value_fn:
        model = ValueMLP(
            n_layers=args.n_layers, fp_dim=args.fp_dim, latent_dim=args.latent_dim, dropout_rate=0.1, device=device
        ).to(device)
    ...
    result = {"succ": [], "cumulated_time": [], "iter": [], "routes": [], "route_costs": [], "route_lens": []}
    ...
    for i, route in enumerate(routes):
        ...
        logging.info(
            "Succ: %d/%d/%d | avg time: %.2f s | avg iter: %.2f" % (tot_succ, tot_num, num_targets, avg_time, avg_iter)
        )
