...
def train():
    ...
    model = ValueMLP(n_layers=args.n_layers, fp_dim=args.fp_dim, latent_dim=args.latent_dim, dropout_rate=0.1, device=device)

    assert os.path.exists("%s/%s.pt" % (args.value_root, args.value_train))

    train_data_loader = ValueDataLoader(fp_value_f="%s/%s" % (args.value_root, args.value_train), batch_size=args.batch_size)

    val_data_loader = ValueDataLoader(fp_value_f="%s/%s" % (args.value_root, args.value_val), batch_size=args.batch_size)
